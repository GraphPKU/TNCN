...
def train():
    ...
    for batch in tqdm(train_loader):
        ...
        _, hashes, cards = heuristicModel(z, edge_index)

        pos_elph_feats = heuristicModel.elph_hashes.get_subgraph_features(torch.stack([src_re,pos_re]).transpose(1,0), hashes, cards)
        neg_elph_feats = heuristicModel.elph_hashes.get_subgraph_features(torch.stack([src_re,neg_re]).transpose(1,0), hashes, cards)

        # print(pos_elph_feats.shape, neg_elph_feats.shape)

        pos_out = model['link_pred'](z, torch.stack([src_re,pos_re]), pos_elph_feats)
        neg_out = model['link_pred'](z, torch.stack([src_re,neg_re]), neg_elph_feats)
...
def test(loader, neg_sampler, split_mode):
    ...
    for pos_batch in tqdm(loader):
        ...
        for idx, neg_batch in enumerate(neg_batch_list):
            ...
            elph_feats = heuristicModel.elph_hashes.get_subgraph_features(torch.stack([assoc[src], assoc[dst]]).transpose(1,0), hashes, cards)

            # print(pos_elph_feats.shape, neg_elph_feats.shape)

            y_pred = model['link_pred'](z, torch.stack([assoc[src], assoc[dst]]), elph_feats)
...
heuristicModel = ELPH().to(device)
link_pred = LinkPredictor(EMB_DIM + heuristicModel.num_layers*(heuristicModel.num_layers + 2)).to(device)
