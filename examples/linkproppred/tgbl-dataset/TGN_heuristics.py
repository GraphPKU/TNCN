...
from modules.ELPH.elph import ELPH
from modules.ELPH.heuristics import CN, RA, AA, PPR
from modules.ELPH.decoder import LinkPredictor, LinkPredictor_h
    ...
    for batch in tqdm(train_loader):
        ...
        def generate_adj_1_hop():
            ...
            if edge_index.size(1) == 0:
                ...
                adj = sparse.csr_matrix((torch.ones(edge_index.size(1)*2).numpy(), (torch.cat((edge_index[0], edge_index[1])).cpu().numpy(), torch.cat((edge_index[1], edge_index[0])).cpu().numpy())), shape=(id_num, id_num)).tocsr()
            return adj
        
        pos_curr_links = torch.stack([src_re, pos_re]).cpu().transpose(0,1)
        neg_curr_links = torch.stack([src_re, neg_re]).cpu().transpose(0,1)
        adj = generate_adj_1_hop()
        # pcn, pra, paa, pppr = CN(adj, pos_curr_links)[0], RA(adj, pos_curr_links)[0], AA(adj, pos_curr_links)[0], PPR(adj, pos_curr_links)[0]
        # ncn, nra, naa, nppr = CN(adj, neg_curr_links)[0], RA(adj, neg_curr_links)[0], AA(adj, neg_curr_links)[0], PPR(adj, neg_curr_links)[0]
        # pos_heu_feats = torch.stack([pcn, pra, paa, pppr]).transpose(0,1).to(device)
        # neg_heu_feats = torch.stack([ncn, nra, naa, nppr]).transpose(0,1).to(device)
        pppr = PPR(adj, pos_curr_links)[0]
        nppr = PPR(adj, neg_curr_links)[0]
        pos_heu_feats = torch.stack([pppr]).transpose(0,1).to(device)
        neg_heu_feats = torch.stack([nppr]).transpose(0,1).to(device)

        # print(pos_elph_feats.shape, neg_elph_feats.shape)
        # print(pos_heu_feats.shape, neg_heu_feats.shape) 
        pos_out = model['link_pred'](z, torch.stack([src_re,pos_re]), pos_heu_feats)
        neg_out = model['link_pred'](z, torch.stack([src_re,neg_re]), neg_heu_feats)
...
def test(loader, neg_sampler, split_mode):
    ...
    for pos_batch in tqdm(loader):
        ...
        for idx, neg_batch in enumerate(neg_batch_list):
            ...
            adj = generate_adj_1_hop()
            curr_links = torch.stack([assoc[src], assoc[dst]]).cpu().transpose(0,1)
            # cn, ra, aa, ppr = CN(adj, curr_links)[0], RA(adj, curr_links)[0], AA(adj, curr_links)[0], PPR(adj, curr_links)[0]
            # heuristics = torch.stack([cn, ra, aa, ppr]).transpose(0,1).to(device)
            ppr = PPR(adj, curr_links)[0]
            heuristics = torch.stack([ppr]).transpose(0,1).to(device)

            y_pred = model['link_pred'](z, torch.stack([assoc[src], assoc[dst]]), heuristics)
...
heuristic_num = 1
link_pred = LinkPredictor(EMB_DIM + heuristic_num).to(device)
...
results_filename = f'{results_path}/{MODEL_NAME}_{DATA}_{NCN_MODE}_{args.msg_func}_{args.emb_func}_{args.agg_func}_{args.nei_sampler}_results.json'
...
for run_idx in range(NUM_RUNS):
    ...
    save_model_id = f'{MODEL_NAME}_{DATA}_{SEED}_{run_idx}_NCN_{NCN_MODE}_{args.msg_func}_{args.emb_func}_{args.agg_func}_{args.nei_sampler}'
