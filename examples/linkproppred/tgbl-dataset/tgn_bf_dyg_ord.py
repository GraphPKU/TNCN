...
def train():
    ...
    for batch in tqdm(train_loader):
        ...
        z2_n_id, z2_edge_index, z2_e_id = train_find_neighbor(train_neighbor_sampler, pos_n_id, pos_n_t, 1)
        assoc[z2_n_id] = torch.arange(z2_n_id.size(0), device=device)
        z2, _ = model['memory'](z2_n_id)
        z2 = model["ord_func"](z2, z2_edge_index, train_data.msg[z2_e_id].to(device))
        ...
        pos_out = model['link_pred'](pos_z1[src_re], pos_z1[pos_re], z2_src, z2_pos_dst)

        neg_n_id, neg_unique_idx = np.unique(torch.cat([neg_src, neg_dst]).cpu().numpy(), return_index=True)
        neg_n_t = n_t[neg_unique_idx]

        z2_n_id, z2_edge_index, z2_e_id = train_find_neighbor(train_neighbor_sampler, neg_n_id, neg_n_t, 1)
        assoc[z2_n_id] = torch.arange(z2_n_id.size(0), device=device)
        z2, _ = model['memory'](z2_n_id)
        z2 = model["ord_func"](z2, z2_edge_index, train_data.msg[z2_e_id].to(device))
        ...
        neg_out = model['link_pred'](neg_z1[nsrc_re], neg_z1[neg_re], z2_neg_src, z2_neg_dst)
...
def test(loader, neg_sampler: NegativeEdgeSampler, split_mode):
    ...
    for idx, pos_batch in enumerate(tqdm(loader)):
        ...
        z2_n_id, z2_edge_index, z2_e_id = test_find_neighbor(full_neighbor_sampler, pos_n_id, pos_n_t, 1)
        assoc[z2_n_id] = torch.arange(z2_n_id.size(0), device=device)
        z2, _ = model['memory'](z2_n_id)
        ...
        pos_pred = model['link_pred'](pos_z1[src_re], pos_z1[pos_re], z2_pos_src, z2_pos_dst)

        neg_n_id, neg_unique_idx = np.unique(torch.cat([neg_src, neg_dst]).cpu().numpy(), return_index=True)
        neg_n_t = n_t[neg_unique_idx]

        z2_n_id, z2_edge_index, z2_e_id = test_find_neighbor(full_neighbor_sampler, neg_n_id, neg_n_t, 1)
        assoc[z2_n_id] = torch.arange(z2_n_id.size(0), device=device)
        z2, _ = model['memory'](z2_n_id)
        z2 = model["ord_func"](z2, z2_edge_index, data.msg[z2_e_id].to(device))
        z2_neg_src, z2_neg_dst = z2[assoc[neg_src]], z2[assoc[neg_dst]]
        ...
        neg_pred = model['link_pred'](neg_z1[nsrc_re], neg_z1[neg_re], z2_neg_src, z2_neg_dst)
...
from modules.decoder import LinkPredictor_h_ORD
link_pred = LinkPredictor_h_ORD(EMB_DIM).to(device)
