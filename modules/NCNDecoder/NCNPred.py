...
class NCNPredictor(torch.nn.Module):
    ...
    def __init__(self,
        ...
        super().__init__()
        assert NCN_mode in [0, 1, 2], 'Invalid NCN Mode! Mode must be 0, 1, or 2.'
        k = NCN_MODE_HOP_TAB[NCN_mode]
    ...
    def multidomainforward(self,
        ...
        cn_emb = get_cn_emb(x, adjs, tar_ei, NCN_mode, cn_time_decay, time_info)
        xs = torch.cat([xij, cn_emb], dim=-1)
        xs = self.xsmlp(xs)

        return xs

    def forward(self, x, adj, tar_ei, NCN_mode, cn_time_decay=False, time_info=None):#, z=None):
        return self.multidomainforward(x, adj, tar_ei, NCN_mode, cn_time_decay, time_info)#, z)


class NCNPredwithAttr(torch.nn.Module):
    cndeg: Final[int]
    def __init__(self,
                 in_channels,
                 hidden_channels,
        ...
        self.xslin = nn.Linear(k * in_channels, 2 * in_channels)
        self.xzmlp = nn.Sequential(
            nn.Linear(4 * in_channels, hidden_channels),
            nn.ReLU(),
            nn.Linear(hidden_channels, out_channels)
        )
    ...
    def multidomainforward(self,
        ...
        xj = x[tar_ei[1]]
        zi, zj = z
        zij = torch.mul(zi, zj).reshape(-1, zi.size(1))
        xij = torch.mul(xi, xj).reshape(-1, x.size(1))
...
class NCNPredWOW(torch.nn.Module):
    cndeg: Final[int]
    def __init__(self,
                 in_channels,
                 hidden_channels,
...
def get_cn_emb(x, edge_index, tar_ei, NCN_mode, cn_time_decay=False, time_info=None):
