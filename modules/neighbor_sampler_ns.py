...
def get_neighbor_sampler(data: TemporalData, sample_neighbor_strategy: str = 'recent', time_scaling_factor: float = 0.0, seed: int = None):
    """
    ...
    :return:
    """
    max_node_id = max(data.src.max(), data.dst.max()).item()
    ...
    for src_node_id, dst_node_id, edge_id, node_interact_time in zip(data.src.cpu().numpy(), data.dst.cpu().numpy(), np.arange(1, len(data.src) + 1, dtype=np.int64), data.t.cpu().numpy()):
...
class NeighborSampler:
    ...
    def get_historical_neighbors(self, node_ids: np.ndarray, node_interact_times: np.ndarray, num_neighbors: int = 10):
        """
        ...
        mask = (nodes_neighbor_ids > 0)
        nodes_neighbor_ids = nodes_neighbor_ids[mask]
        nodes_edge_ids = nodes_edge_ids[mask]
        nodes_neighbor_times = nodes_neighbor_times[mask]
        # nodes_neighbor_ids = nodes_neighbor_ids.astype(np.longlong).flatten()
        # nodes_edge_ids = nodes_edge_ids.astype(np.longlong).flatten()
        # nodes_neighbor_times = nodes_neighbor_times.astype(np.float32).flatten
        return nodes_neighbor_ids, nodes_edge_ids, nodes_neighbor_times

    def get_historical_neighbors_format(self, node_ids: torch.Tensor, node_interact_times: torch.Tensor, num_neighbors: int = 20):
        """
        get historical neighbors of nodes in node_ids with interactions before the corresponding time in node_interact_times
        ...
        node_interact_times = node_interact_times.cpu().numpy()
        ...
        mask = (nodes_neighbor_ids > 0)
        e_id = nodes_edge_ids[mask]
        uni_e_id, index_e_id = np.unique(e_id, return_index=True)
        dst = nodes_neighbor_ids[mask][index_e_id]
        src = np.repeat(node_ids, num_neighbors)[mask.flatten()][index_e_id]
        edge_index = np.stack([src, dst], axis=0)
        n_id = np.unique(np.concatenate([src, dst]))
        # print(edge_index.shape)
        # exit()

        n_id = torch.from_numpy(n_id).long().to(device)
        edge_index = torch.from_numpy(edge_index).long().to(device)
        e_id = torch.from_numpy(uni_e_id).long().to(device)
        return n_id, edge_index, e_id
...
class NegativeEdgeSampler(object):
    ...
    def __init__(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, interact_times: np.ndarray = None, last_observed_time: float = None,
                 negative_sample_strategy: str = 'random', seed: int = None, device: str = 'cpu'):
        """
    ...
    def sample(self, size: int, batch_src_node_ids: np.ndarray = None, batch_dst_node_ids: np.ndarray = None,
        ...
        negative_src_node_ids = torch.from_numpy(negative_src_node_ids).long().to(self.device)
        negative_dst_node_ids = torch.from_numpy(negative_dst_node_ids).long().to(self.device)
